...
fn check_for_image(channel: Channel) -> i32 {
    ...
    let imagereport = match channel.image {
        ...
        None => match channel.itunes_ext() {
            None => {
                errors += 1;
                "no <image> or itunes extensions found".red()
            }
            Some(ext) => match ext.image() {
                None => {
                    errors += 1;
                    "no <image> or <itunes:image> tag found".red()
                }
                Some(imageurl) => {
                    format!("<itunes:image> tag with url:\n    \"{}\"", imageurl).green()
                }
            },
        },
    };
...
fn main() {
    ...
    for i in 0..numitems {
        ...
        let title = match &item.title {
            Some(title) => format!("({}/{}) \"{}\"", i + 1, numitems, title).green(),
            None => {
                errors += 1;
                format!("({}/{}) No title!", i + 1, numitems).red()
            }
        };

        // requires an <enclosure>
        let report = match &item.enclosure {
            Some(enclosure) => format!(
                "    url:  {}\n    type: {}\n    length: {} bytes",
                enclosure.url, enclosure.mime_type, enclosure.length
            )
            .green(),
            None => {
                errors += 1;
                "    No enclosure found".red()
            }
        };
        println!("{}", title);
        println!("{}", report);
